...
def mp3download(sname,cc,ssrname=''):
    print("第",cc,"首歌：",sname)
...
#输入需要请求的关键词
songs=open('歌曲.txt','r',encoding='utf-8')
...
for song in ssgs:
  ...
  if longname != sname:
      ...
      mp3download(sname,cc, ssrname)
  else:
      mp3download(sname,cc) 
  cc= cc + 1     
songs.close() 
# for song in songres:
#     sname = song.group("ssg")
#     print("第",cc,"首歌：",sname)
#     #定义搜索链接，通过拼接搜索前导+歌曲名来搜索歌曲
#     searchurl="https://zz123.com/search/?key="+sname
...
#             sgname=item.group("sgname") #歌曲名称
#             srname=item.group("srname") #歌手名称
#             if (sgname == sname) | (sgname == sname + srname) | (sgname == srname + sname):
#                 #拼接歌曲下载地址
#                 songurl="https://zz123.com/xplay/?act=songplay&id="+songtag
#                 #检差歌曲文件夹是否存在
#                 fname=srname+'\\'+sname
